...
class BrapTools():
  ...
  def search_revenue(data):
    ...
    question you have from it.
		For example, `PETR4|what was last quarter's revenue`.
    """
    stock, ask = data.split("|")
    queryApi = QueryApi(api_key=os.environ['BRAP_API_KEY'])
    query = {
      "query": {
        "query_string": {
    ...
    answer = BrapTools.__embedding_search(link, ask)
    return answer

  @tool("Search 10-K form")
  def search_10k(data):
    """
    Useful to search information from the balance sheet form for a
    given stock.
    The input to this tool should be a pipe (|) separated text of
    length two, representing the stock ticker you are interested, what
    question you have from it.
    For example, `PETR4|what was last year's revenue`.
    """
    stock, ask = data.split("|")
    queryApi = QueryApi(api_key=os.environ['BRAP_API_KEY'])
    query = {
  ...
  def __embedding_search(url, ask):
    text = BrapTools.__download_form_html(url)
